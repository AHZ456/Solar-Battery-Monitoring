...
void app_main(void)
{
    ...
    while (1)
    {
        ...
        if (ret == ESP_OK || ret == ESP_ERR_INVALID_STATE)
        {
            ...
            adc_digi_output_data_t **p = malloc(sizeof(adc_digi_output_data_t *) * TIMES);
            // uint16_t** p=malloc(sizeof(uint16_t*)*TIMES);

            for (int i = 0, j = 0; i < ret_num; i += ADC_RESULT_BYTE, j++)
            {
                (p[j]) = (void *)&result[i];

                holder += ((p[j])->type1.data);
            }
            var = holder;

            holder /= ((ret_num / ADC_RESULT_BYTE) + 1);

            vout = ((holder + 1) * 1.086) / 4095;

#if CONFIG_IDF_TARGET_ESP32
            // printf("var: %u, Channel: %u, Value: %u , Voltage: %lf \n", var, (p[0])->type1.channel, holder+1,vout);
            printf("Value: %u  \n", holder + 1);
            ...
            if (ADC_CONV_MODE == ADC_CONV_BOTH_UNIT || ADC_CONV_MODE == ADC_CONV_ALTER_UNIT)
            {
                ...
                {
                    ESP_LOGI(TAG, "Unit: %d,_Channel: %d, Value: %d", p[0]->type2.unit + 1, p[0]->type2.channel, (p[0])->type2.data);
                }
                else
                {
                    // abort();
                    ESP_LOGI(TAG, "Invalid data [%d_%d_%d]", p[0]->type2.unit + 1, p[0]->type2.channel, p[0]->type2.data);
                }
            }
#if CONFIG_IDF_TARGET_ESP32S2
            else if (ADC_CONV_MODE == ADC_CONV_SINGLE_UNIT_2)
            {
                ESP_LOGI(TAG, "Unit: %d, Channel: %d, Value: %x", 2, p[0]->type1.channel, p[0]->type1.data);
            }
            else if (ADC_CONV_MODE == ADC_CONV_SINGLE_UNIT_1)
            {
                ESP_LOGI(TAG, "Unit: %d, Channel: %d, Value: %x", 1, p[0]->type1.channel, p[0]->type1.data);
            }
