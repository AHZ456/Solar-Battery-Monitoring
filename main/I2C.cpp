...
{
#if SHUNT_CKT
  SET_READ(addr, CUR_REG);
#elif LPA_CKT || HALL_CKT
...
void INIT()
{
  ...
  SET_REG(addr, CONFIG_REG, DATA1, 2);
  SET_REG(addr, CAL_REG, DATA2, 2);
#elif LAP_CKT
...
int16_t I2C_READ()
{
  ...
  CHECK(err);
/*#if SHUNT_CKT 
  result = (((uint16_t)resultA[0] << 8) | ((uint16_t)resultA[1]))-27;
#elif LPA_CKT
  result = (((uint16_t)resultA[0] << 8) | ((uint16_t)resultA[1]));*/
#if SHUNT_CKT || LPA_CKT
result = (((uint16_t)resultA[0] << 8) | ((uint16_t)resultA[1]));
#elif HALL_CKT
          ...
          esp_adc_cal_characteristics_t *adc_chars, int NO_OF_SAMPLES)
{
  ...
  double cf = calv1 * (260 + 10000) / 260;                                              // GPIO34 (24 V)
  double cf2 = calv2 * (470 + 10000) / 470;                                             // GPIO35 (12 V)
#if SHUNT_CKT
  SET_READ(addr, VS_REG);
  voff = (double)I2C_READ() / VLSB;

#endif
  MEAS batteries = {(voltage)*cf - (voltage2)*cf2, (voltage2)*cf2 - voff, voff};
  return batteries;
}
...
/*void SEND(MEAS value,float Charge1){
    ...
    feed.fields.push_back({1, value.v1}); //< field1 = 11.11f
